#!/usr/bin/env bash
# Resolve the Python that the packaging wheel stage will use, WITHOUT assuming
# any particular environment manager (uv/venv, conda, pyenv, system, ...).
# Meant to be `source`d.
#
# Contract: the caller has the CARLA client environment ACTIVE (its `python3`
# on PATH), or has arranged one of the optional auto-pickups below. The wheel
# stage runs `python3 -m build`, so `python3` must import `build`. We verify
# that here, before the expensive editor compile, and otherwise stay out of the
# way — no version is pinned and no interpreter is renamed unless the caller
# explicitly sets CARLA_PY_VERSION.
#
...
# Pick the interpreter the wheel stage will call and verify it can `import
# build`. Sets CARLA_PY_BIN and CARLA_PY_ARG:
