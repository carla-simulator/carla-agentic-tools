#!/usr/bin/env bash
# Self-contained environment for the replay-recording skill (RUNTIME/client).
...
#
# NOTE: replay reads the .log from the SERVER's filesystem. A relative name is
# resolved under the server's CarlaUE4/Saved/ directory; pass an absolute path.
# On a remote server the file must exist on that machine, not here.
