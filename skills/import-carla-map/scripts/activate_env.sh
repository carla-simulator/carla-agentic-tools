#!/usr/bin/env bash
# Resolve the Python that Import.py will use, WITHOUT assuming any
...
#   1. direnv is only consulted when an .envrc actually governs `from`. Run from
#      a directory OUTSIDE the checkout's .envrc tree (this repo is standalone,
#      so `from` often is), `direnv export bash` does not load anything -- it
#      UNLOADS, emitting `unset CARLA_UE4_ROOT; unset UE4_ROOT; unset
#      VIRTUAL_ENV` to revert the .envrc the caller's shell has loaded.
#   2. An explicit export always wins. Whatever the hooks say, the roots the
#      caller passed in are restored afterwards.
carla_load_local_env() {
  local from="${1:-.}"
  # Snapshot the caller's explicit values before any hook can touch them.
  local _keep_carla="${CARLA_UE4_ROOT:-}" _keep_ue4="${UE4_ROOT:-}"
  ...
  if command -v direnv >/dev/null 2>&1 && _carla_has_envrc "${from}"; then
    local exports
    exports="$(cd "${from}" 2>/dev/null && direnv export bash 2>/dev/null)" || true
    [ -n "${exports:-}" ] && eval "${exports}"
  fi

  [ -n "${_keep_carla}" ] && export CARLA_UE4_ROOT="${_keep_carla}"
  [ -n "${_keep_ue4}" ] && export UE4_ROOT="${_keep_ue4}"
  return 0
}

# Is there an .envrc at or above `from`? Only then is direnv's output a load
# rather than an unload.
_carla_has_envrc() {
  local d
...
#     the import is NOT told a version and the active env wins.
...
carla_require_client_python() {
  ...
  if ! "${CARLA_PY_BIN}" -c 'import carla' 2>/dev/null; then
    ...
    echo "[env] install it there (build-carla-ue4 step 04). See references/maps.md M4." >&2
    return 1
  fi
