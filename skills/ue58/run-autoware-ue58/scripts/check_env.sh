#!/usr/bin/env bash
# Prerequisite checks for run-autoware-ue58 (CARLA on UE 5.8). Read-only, no sudo, fast —
...
echo "== Walker registry and skeletons =="
CONTENT="${CARLA_UE58_ROOT}/Unreal/CarlaUnreal/Content/Carla"
WP="${CONTENT}/Config/WalkerParameters.json"
if [ -f "${WP}" ]; then
  ok "WalkerParameters.json present ($(grep -o '"Id"' "${WP}" | wc -l) walker entries)"
  [ -f "${WP}.bak" ] && warn "  a WalkerParameters.json.bak is left over from an earlier run"
else
  bad "WalkerParameters.json missing — clone carla-content branch ue58-dev-carla"
fi
# The shared skeletons are what makes an imported walker animate; binding to a
# private skeleton fails SILENTLY, so their presence is a hard prerequisite.
DEFS="${CONTENT}/Static/Pedestrian/00_GenericComponents/Definitions"
for g in G2 G3; do
  if [ -f "${DEFS}/Skel_Pedestrian_${g}.uasset" ]; then
    ok "shared skeleton Skel_Pedestrian_${g}"
  else
    bad "Skel_Pedestrian_${g}.uasset missing under Static/Pedestrian/00_GenericComponents/Definitions"
  fi
done
[ -d "${CONTENT}/Blueprints/Walkers" ] \
  && ok "donor blueprints ($(ls -1 "${CONTENT}/Blueprints/Walkers"/BP_*.uasset 2>/dev/null | wc -l) BP_* assets)" \
  || bad "Content/Carla/Blueprints/Walkers missing — no donor blueprint to duplicate"

echo "== Display (a FULL editor is required) =="
# Skeletal FBX export asserts and segfaults in a -run=pythonscript commandlet
# under both -nullrhi and -RenderOffScreen, so this skill drives a real editor.
if [ -n "${DISPLAY:-}" ]; then
  ok "DISPLAY=${DISPLAY}"
else
  bad "DISPLAY unset — export/import need a full editor (a commandlet segfaults on skeletal meshes)"
fi

echo "== Python API (needed only by verify --spawn) =="
if "${PYTHON}" -c 'import carla' 2>/dev/null; then
  ok "carla importable ($("${PYTHON}" -c 'import carla; print(carla.__file__)'))"
else
  warn "carla not importable — plan/import/list work, verify --spawn does not"
  warn "  build target: carla-python-api-install"
fi

echo "== ROS 2 in this build (compile-time, not a runtime flag) =="
if carla_ue58_configured; then
  R2="$(carla_ue58_cmake_opt ENABLE_ROS2)"
  case "${R2}" in
    ON|on|1|TRUE|True) ok "ENABLE_ROS2=${R2} — the native ROS 2 interface is compiled in" ;;
    "") warn "ENABLE_ROS2 not in the cache; re-configure with -DENABLE_ROS2=ON" ;;
    *)  bad "ENABLE_ROS2=${R2} — Autoware talks to CARLA over DDS only; rebuild with -DENABLE_ROS2=ON" ;;
  esac
else
  warn "tree not configured — cannot tell whether ROS 2 is compiled in"
fi

echo "== Autoware integration shipped with this checkout =="
AV="${CARLA_UE58_ROOT}/PythonAPI/examples/av_stacks/autoware"
if [ -d "${AV}" ]; then
  ok "av_stacks/autoware present"
  for f in run/run_carla_autoware.sh run/spawn_vad_rig.py map_tools/generate_map_artifacts.py \
           install/install_autoware.sh README.md; do
    [ -e "${AV}/${f}" ] && ok "  ${f}" || warn "  ${f} MISSING"
  done
else
  bad "PythonAPI/examples/av_stacks/autoware missing — this checkout has no Autoware integration"
fi
[ -f "${CARLA_UE58_ROOT}/PythonAPI/examples/autoware_demo.py" ] \
  && ok "autoware_demo.py present (the CARLA-side driver)" \
  || bad "PythonAPI/examples/autoware_demo.py missing"

echo "== Autoware itself (external, NOT part of CARLA) =="
AWS="${AUTOWARE_WS:-${HOME}/autoware}"
if [ -d "${AWS}/install" ]; then
  ok "source workspace at ${AWS} (install/ present)"
else
  warn "no Autoware source workspace at ${AWS} — set AUTOWARE_WS, or use --stack docker"
fi
if command -v docker >/dev/null 2>&1; then
  IMG="$(docker images --format '{{.Repository}}:{{.Tag}}' 2>/dev/null | grep -m1 'autoware' || true)"
  [ -n "${IMG}" ] && ok "docker image ${IMG}" || warn "no local autoware docker image"
else
  warn "docker not on PATH — --stack docker is unavailable"
fi

echo "== Map artifacts (Autoware needs its own, not CARLA's .xodr) =="
FOUND=0
for d in "${AV}/map_tools/maps"/*; do
  [ -d "${d}" ] || continue
  if [ -f "${d}/lanelet2_map.osm" ] && [ -f "${d}/pointcloud_map.pcd" ]; then
    ok "$(basename "${d}"): lanelet2_map.osm + pointcloud_map.pcd"
    FOUND=1
  fi
done
[ "${FOUND}" -eq 1 ] || warn "no generated map set found — run: bash scripts/autoware.sh maps --town Town10HD_Opt"
